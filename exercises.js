...
function newUser(name, email, password) {
  ...
  var obj={
  name:"abhisekh",
    email:"gautamabish683@gmail.com",
    password:"5805593031"
  }
return obj;
  }
...
function setUsersToPremium(users) {
  ...
  for (var start=0;start<users.length;start++){
  users[start]['isPremium']=true;
  }
  return users;
}
...
function sumUserPostLikes(user) {
  ...
  var total=0;
  for (var start=0;start<users.length;start++){
 for (var begin=0;begin<users[start][posts].length;begin++){
 total=total+users[start][posts][begin]['likes']+total;
 }
  }
  return total;
}
